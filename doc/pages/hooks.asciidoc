...
Hooks provide additional value expansions for the parameter value
`hook_param` and its regex captures `hook_param_capture_N` (see
<<expansions#value-expansions,`:doc expansions value-expansions`>>).
